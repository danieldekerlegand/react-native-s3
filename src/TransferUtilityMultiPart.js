...
const { RNS3TransferUtilityMultiPart } = NativeModules;
...
export default class TransferUtilityMultiPart {
	async setupWithNative() {
		const result = await RNS3TransferUtility.setupWithNative();
		if (result) {
			await getTaskExtras();
			RNS3TransferUtilityMultiPart.initializeRNS3();
		}
		return result;
	}

	async setupWithBasic(options = {}) {
		console.log("setupWithBasic");
		...
		const result = await RNS3TransferUtilityMultiPart.setupWithBasic({ ...defaultOptions, ...opts});
		if (result) {
			await getTaskExtras();
			RNS3TransferUtilityMultiPart.initializeRNS3();
		}
	...
	async setupWithCognito(options = {}) {
		...
		const result = await RNS3TransferUtilityMultiPart.setupWithCognito({ ...defaultCognitoOptions, ...opts });
		if (result) {
			await getTaskExtras();
			RNS3TransferUtilityMultiPart.initializeRNS3();
		}
	...
	enableProgressSent(enabled) {
		return RNS3TransferUtilityMultiPart.enableProgressSent(enabled);
	}
	...
	async upload(options = {}, others = {}) {
		...
		const task = await RNS3TransferUtilityMultiPart.upload({
			...opts,
	...
	async download(options = {}, others = {}) {
		const opts = snakeCaseKeys(options);
		const task = await RNS3TransferUtilityMultiPart.download({
			...options,
			file: normalizeFilePath(opts.file)
		});
		const extra = {
	...
	pause(id) {
		RNS3TransferUtilityMultiPart.pause(Number(id));
	}

	resume(id) {
		RNS3TransferUtilityMultiPart.resume(Number(id));
	}

	cancel(id) {
		RNS3TransferUtilityMultiPart.cancel(Number(id));
	}
	...
	async deleteRecord(id) {
		...
		return RNS3TransferUtilityMultiPart.deleteRecord(Number(id));
	}

	async getTask(id) {
		const task = await RNS3TransferUtilityMultiPart.getTask(Number(id));
		if (task) {
			return putExtra(task);
		}
		return null;
	...
	async getTasks(type = "", idAsKey) {
		if (transferTypes.indexOf(type) > -1) {
			let tasks = await RNS3TransferUtilityMultiPart.getTasks(type);
